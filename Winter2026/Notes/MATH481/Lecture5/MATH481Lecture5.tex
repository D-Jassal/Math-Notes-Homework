...
\begin{remark}
    ...
    \]
    However $L_\Lambda$ and $L_\mu$ are much more tractable objects adn we will compute them in the next chapter.
\end{remark}
\pf[Proof of Theorem 2.13]{
    Given $f:\N\to\C$ set 
    \[
        S_f(x)=\sum_{f(n)}\quad\quad T_f(x)=\sum_{n\leq x}\frac{f(n)}{n}
    \]
    \[
    ...
    \]
    We need to show that $\lim_{x\to\infty}=M_f$.
    \begin{align*}
