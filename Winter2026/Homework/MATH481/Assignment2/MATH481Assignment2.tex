...
\begin{align*}
	...
	&=(\log x+\gamma)\left(\log\sqrt{x}+\gamma+O\left(\frac{1}{x}\right)\right)-\sum_{a\leq \sqrt{x}}\frac{\log a}{a}+O\left(\frac{1}{\sqrt{x}}\right)\\
	&=(\log x+\gamma)\left(\frac{1}{2}\log x+\gamma+O\left(\frac{1}{x}\right)\right)-\sum_{a\leq \sqrt{x}}\frac{\log a}{a}+O\left(\frac{1}{\sqrt{x}}\right)\\
	&=\frac{1}{2}\log^2 x+\frac{3\gamma}{2}\log x+\gamma^2-\sum_{a\leq \sqrt{x}}\frac{\log a}{a}+O\left(\frac{\log x}{x}\right).
\end{align*}
Using Abel's summation formula for the sum with $a(n)=1$, and $f(n)=\frac{\log n}{n}$.
\begin{align*}
	S_1(x)&=\sum_{a\leq \sqrt{x}}\frac{\log a}{a}\\
	&=\floor{\sqrt{x}}\frac{\log x}{x}-\int_{1}^{\sqrt{x}}\floor{t}\frac{1-\log t}{t^2}\,dt\\
	&=\sqrt{x}-\fract{\sqrt{x}}\frac{\log x}{x}-\int_{1}^{\sqrt{x}}(t-\fract{t})\frac{1-\log t}{t^2}\,dt.
\end{align*}
...
\stepcounter{section}
\section*{Problem 5 [10 Marks]} Using only Mertens' type estimates (but not the PNT), obtain an asymptotic estimate for the partial sums
$\displaystyle{S(x) =\sum_{p\leq x;\; p\;\text{prime} }\frac{1}{p\log p}}$ with as good an error term as you can get using only results at the level of Mertens.
