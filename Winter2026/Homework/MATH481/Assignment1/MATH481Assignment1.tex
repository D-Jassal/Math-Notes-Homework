...
\begin{proof}
	Since $p(x)$ has integer coefficients and the integers are closed under addition and multiplication, then for each integer input for $p(x)$ we will have an integer output. Find $m\in\Z$ such that $p(m)\neq0,\pm1$, this is possible since $p(x)$ has degree greater than 1, so it cannot be oscillate between -1,0,1 and will eventually start to grow in absolute value. There exists $q$ prime such that $q\mid p(m)$. Let $m_k'=m+kp$ with $k\in\N$ then with 
	\[
	...
	\]
	Thus, each value of $p(m_k')$ is divisible by $p$. We can only have $p\mathrel\Vert p(m_k')$ for finitely many $k$ because $\lim_{x\to\infty}p(x)=\infty$.
\end{proof}
...
Thus, for all $a$ coprime to $k$ we have that $f(a)$ is a $k-$th root of infinity. 
\stepcounter{section}
