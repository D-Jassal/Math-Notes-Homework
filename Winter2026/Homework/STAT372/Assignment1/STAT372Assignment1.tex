...
Let the observed scrap lengths be $x_1, x_2, \dots, x_n > 0$. This has to be greater than zero because a scrap cannot have length zero as it would then not be a scrap. The data-generation process implies that each shorter piece satisfies
    \[
        0 < X \leq \frac{\theta}{2}.
    \]
...
From the data we see that $x_{\text{max}}=3\frac{6}{32}$. Then
    \[
        \colorbox{yellow}{$\widehat{\theta}=2\times3\frac{6}{32}=6\frac{3}{8}$ inches}
    \]
\end{document}