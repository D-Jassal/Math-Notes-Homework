...
\begin{proof}
	An element $y\in G.x$ looks like $y=gx$ for some $g\in G$. Elements $h\in G_x$ have the unique property that $hx=x$ for all $h\in G_x$. For some arbitrary element $y\in G.x$ we have 
	\begin{align*}
		h\cdot y&=h\cdot (y \cdot x)\\
		&=(hg)\cdot x\\
		&=(g^{-1}hg)\cdot x.
	\end{align*}
	Since $h\in G_x$ and $G_x$ is a proper normal subgroup we have $g^{-1}hg\in G_x$. Since $G_x$ is the stabilizer of $x$ we have
	\[
		(g^{-1}hg)\cdot x=x.
	\]
\end{proof}
...
\begin{enumerate}[label=(\alph*)]
	...
	\begin{proof}
		Since $N\unlhd G$ we have $aN=Na\,\forall a\in G$. Since $M\leq G \Rightarrow M\subseteq G$, and $N\leq M$, then for all $m\in M$ we have $mN=Nm$. Therefore, $N\unlhd M.$
	\end{proof}
	\item Show that $(G/N )/(M/N ) \cong G/M $.
	\begin{proof}
		The quotient groups $G/N,M/N,G/M$ are well defined as the group's are normal in the correct order. Define $\varphi: G/N\mapsto G/M$ by 
		\[
			\varphi(gN)=gM.
		\]
		\begin{claim}
		...
		\begin{proof}[Proof of claim.]
			For any $gM\in G/M$, we have $gN\in G/N$ with $\varphi(gN)=gM$.\\
			Let $g_1N,g_2N\in G/N$ then 
			\[
				\varphi(g_1Ng_2N)=\varphi(g_1g_2N)=g_1g_2M
			\]
			and
			\[
				\varphi(g_1N)\varphi(g_2N)=g_1Mg_2M=g_1g_2M=\varphi(g_1Ng_2N). \qedhere
			\]
		\end{proof}
		The kernel of $\varphi$ is 
		\[
			\ker\varphi=\left\{ gN\in G/N : \varphi(gN)=gM=M \right\},
		\]
		but if $gM=M$, then $g\in M$. So we can rewrite the kernel as
		\[
			\ker\varphi=\left\{ gN\in G/N : g\in M \right\}.
		\]
		That is kernel consists of all $gN$ such that $g\in M$, which is precisely $M/N$ viewed as a subgroup of $G/N.$
		\[
			(G/N )/(M/N ) \cong G/M.
		\]
	\end{proof}
