...
\begin{proof}
	...
	\[
		r^{ln}\equiv r^k \Leftrightarrow x^n\equiv a \mod m\qedhere
	\]
\end{proof}
\stepcounter{section}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{proof}
		...
		\[
			k=t(p^{h-2}(p-1)),\, 1\leq t\leq p-1. \qedhere
		\]
	\end{proof}
    ...
    \begin{proof}
		...
		\[
			k=\frac{t}{2}(p^{h-2}(p-1)),\, 1\leq t\leq p-1. \qedhere
		\]
	\end{proof}
\end{enumerate}
...
\begin{proof}
	...
	\[
		\prod_{r=0}^{\varphi(n)-1}g^r=g^{1+2+\cdots+\varphi(n)-1}=g^{\frac{(\varphi(n)-1)\varphi(n)}{2}}\equiv -1\mod n.	\qedhere
	\]
\end{proof}
...
\begin{proof}
	...
	\]
	Then $a\equiv 0 \mod q,$ and $a\equiv 0\mod p_i$ ($1\leq i \leq n$). Since $N>1$ we have $\exists p$ prime such that $p\vert N$.\\
	Then $N\equiv -1\mod q$ and $N\equiv -1\mod p_i$ ($1\leq i \leq n$). So we have $p\neq q$ and $p\not\in \{p_1,p_2,\dots,p_n\}$. In 7b we showed that all prime divisors of $N$ are of the form $p=q$ or $p\equiv 1 \mod q$. Since $p\neq q$ it must be that $p\equiv 1\mod q$. But $p\not\in \{p_1,p_2,\dots,p_n\}$, so the assumption is false.
\end{proof}
...
\] 
\begin{proof}
	By \textit{theorem 3.1.8} we know that 
	\[ 
		\left(-1\right)^{\frac{p-1}{2}}=\begin{cases} 1\:\:\: \text{if}\:\:\: p\equiv 1\mod{4}\\ -1\:\:\: \text{if}\:\:\: p\equiv 3\mod{4}.\end{cases}
	\] 
	Since $p\geq 5$ we have $p\not\equiv 0 \mod3.$\\
	For $p$ modulo 12 we have $p\equiv 1,5,7,11\mod 12$.
	\begin{figure}[h!]
		\centering
		\begin{tabular}{c|c|c|c}
			$p$ mod 12 & $p$ mod 4 & $p$ mod 3 & $\left(\frac{3}{p}\right)$\\
			\hline
			1&1&1&1\\
			5&1&2&-1\\
			7=-5&3&1&-1\\
	...
	So we have 
	\[
		\left(\frac{3}{p}\right)=\begin{cases} 1\:\:\: \text{if}\:\:\: p\equiv \pm1\mod{12}\\ -1\:\:\: \text{if}\:\:\: p\equiv \pm5\mod{12}.\end{cases}\qedhere
	\]
\end{proof}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{proof}
		$a\equiv b\mod n\Leftrightarrow a\equiv b\mod p_i$, where $n=p_1^{e_1}p_2^{e_2}\cdots p_k^{e_k}$, with $p_i$ distinct primes and $e_i\in\Z$ ($1\leq i\leq k$). That is to say that
		\[
		...
		\[
			\left(\frac{a}{n}\right)=\left(\frac{b}{n}\right).\qedhere
		\]
	\end{proof}
    \item If $a$ and $b$ are integers, then $\left(\frac{a}{n}\right)\left(\frac{b}{n}\right)=\left(\frac{ab}{n}\right)$.
    \begin{proof}
		Using the prime factorization of $n$ given in part (a) we have
		\[
		...
		So
		\[
			\left(\frac{a}{n}\right)\left(\frac{b}{n}\right)=\left(\frac{a}{n}\right)=\left(\frac{a}{p_1}\right)^{e_1}\left(\frac{a}{p_i}\right)^{e_2}\cdots\left(\frac{a}{p_k}\right)^{e_k}\left(\frac{b}{n}\right)\left(\frac{b}{p_1}\right)^{e_1}\left(\frac{b}{p_i}\right)^{e_2}\cdots\left(\frac{b}{p_k}\right)^{e_k}=\left(\frac{ab}{n}\right).\qedhere
		\]
	\end{proof}
	\newpage
    ...
    \begin{proof}
		Given that $x^2\equiv a\mod n$ has a solution we have $x^2\equiv 1\mod p_i$ where $p_i$ is from the prime factorization of $n$ given in part (a). We can then see that 
		\[
		\left(\frac{a}{p_i}\right)=1\quad \text{for all } 1\leq i\leq k.
		\]
		Then 
		\[
			\left(\frac{a}{n}\right)=\left(\frac{a}{p_1}\right)^{e_1}\left(\frac{a}{p_i}\right)^{e_2}\cdots\left(\frac{a}{p_k}\right)^{e_k}=1^{e_1}1^{e_2}\cdots1^{e_k}.\qedhere
		\]
	\end{proof}
    \item If $m,n$ are relatively prime odd integers, then \[\left(\frac{m}{n}\right)\left(\frac{n}{m}\right)=(-1)^{\frac{m-1}{2}}(-1)^{\frac{n-1}{2}}.\]
    \begin{proof}
		Let $m=p_1^{e_1}p_2^{e_2}\cdots p_k^{e_k}$ for distinct primes $p_i$ and $e_i\in\Z$ for ($1\leq i\leq r$), and $n=q_1^{f_1}q_2^{f_2}\cdots q_k^{f_k}$ for distinct primes $q_i$ and $f_i\in\Z$ for ($1\leq i\leq r$).\\
		Then,
		\[
		...
		\[
			\left(\frac{m}{n}\right)\left(\frac{n}{m}\right)=\prod_{i=1}^{k}\prod_{j=1}^{r}\left(-1\right)^{e_i f_j\left(\frac{p_i-1}{2}\right)\left(\frac{q_i-1}{2}\right)}		
		\]
		Moving the product into the exponent we obtain
		\[
			\sum_{i=1}^{k}\sum_{j=1}^{r}e_i f_j\left(\frac{p_i-1}{2}\right)\left(\frac{q_i-1}{2}\right)
		\]
		...
		\]
		\textit{This is as far as I got before class started, I'm not so sure that the last step is the right direction to finish the proof.}
	\end{proof}
\end{enumerate}
\end{document}